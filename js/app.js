...
window.PL = (function() {
  ...
  var FEATURES = {
    ...
    'user':         ['parking','maintenance','services','messages','packages','payments'],
  ...
  function demoApi(action, method, body) {
    ...
    switch(action.split('&')[0]) {
      ...
      case 'dashboard':
        ...
        // Query Supabase for pending payment_requests balance
        return fetch(SB_URL + '/rest/v1/payment_requests?select=amount&unit=eq.' + encodeURIComponent(unit) + '&status=eq.pending', {
          headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + SB_KEY }
        }).then(function(r) { return r.json(); })
          .then(function(rows) {
            var bal = 0;
            if (Array.isArray(rows)) rows.forEach(function(r) { bal += parseFloat(r.amount || 0); });
            return {
  ...
  function showModal(type) {
    closeModal();
    if (type === 'login') {
      document.getElementById('loginModal').className = 'modal-overlay show';
      document.getElementById('loginPassGroup').style.display = 'none';
      document.getElementById('loginPass').value = '';
      document.getElementById('loginErr').className = 'auth-error';
      var el = document.getElementById('loginUser');
      if (el) setTimeout(function(){ el.focus(); }, 200);
    } else if (type === 'register') {
      document.getElementById('registerModal').className = 'modal-overlay show';
      // Reset form
      document.getElementById('regFormFields').style.display = '';
  ...
  function closeModal() {
    ...
    modals.forEach(function(m) { m.className = 'modal-overlay'; });
  }

  // ── Landing page parking click ──
  function landingParking() {
    // Show parking booking inside the app without full login
    document.getElementById('landingPage').className = 'screen';
    document.getElementById('appShell').style.display = '';
    // Hide bottom nav for guest mode
    var nav = document.getElementById('bottomNav');
    if (nav) nav.style.display = 'none';
    // Reset building selector (no pre-assigned building for guest)
    var sel = document.getElementById('pkBuilding');
    if (sel) sel.value = '';
    showScreen('parkingBook');
    // Mark as guest parking mode
    window._guestParking = true;
    // Show guest info fields and guest notification
    var gi = document.getElementById('pkGuestInfo');
    if (gi) gi.style.display = '';
    var na = document.getElementById('pkNotifyApp');
    if (na) na.style.display = 'none';
    var ng = document.getElementById('pkNotifyGuest');
    if (ng) ng.style.display = '';
  }

  // Toggle email/phone input based on notification type selection
  function pkToggleNotifyInput() {
    var type = document.getElementById('pkNotifyType').value;
    var emailEl = document.getElementById('pkNotifyEmail');
    var phoneEl = document.getElementById('pkNotifyPhone');
    var smsDisclaimer = document.getElementById('pkSmsDisclaimer');
    emailEl.style.display = type === 'email' ? '' : 'none';
    phoneEl.style.display = type === 'sms' ? '' : 'none';
    if (smsDisclaimer) smsDisclaimer.style.display = type === 'sms' ? '' : 'none';
    if (type === 'email') emailEl.focus();
    if (type === 'sms') phoneEl.focus();
    // Reset consent checkbox when switching
    var cb = document.getElementById('pkSmsConsent');
  ...
  function login() {
    ...
    var passGroup = document.getElementById('loginPassGroup');
    errEl.className = 'auth-error';
    if (!user) { errEl.textContent = 'Enter your access code'; errEl.className = 'auth-error show'; return; }

    api('login', 'POST', { username: user, password: pass }).then(function(d) {
      if (d.error === 'password_required') {
        // User has a password set — show password field
        passGroup.style.display = '';
        document.getElementById('loginPass').focus();
        errEl.textContent = 'Password required for this account';
        errEl.className = 'auth-error show';
        return;
      }
      if (d.error) { errEl.textContent = d.error; errEl.className = 'auth-error show'; return; }
      TOKEN = d.token;
      USER = { token: d.token, user_id: d.user_id || null, user_type: d.user_type, name: d.name, unit: d.unit, address: d.address || '', phone: d.phone, email: d.email };
  ...
  function register() {
    ...
    if (!name || !address || !phone || !pass) {
      errEl.textContent = 'Name, address, phone, and password are required.';
    ...
    api('register', 'POST', { name: name, address: address, unit: unit, phone: phone, email: email, password: pass }).then(function(d) {
      if (d.error) { errEl.textContent = d.error; errEl.className = 'auth-error show'; return; }
      // Show success
  ...
  function changePassword() {
    var currentPass = (document.getElementById('cpCurrent').value || '');
    var newPass = (document.getElementById('cpNew').value || '');
    var newPass2 = (document.getElementById('cpConfirm').value || '');
    var errEl = document.getElementById('cpErr');
    var okEl = document.getElementById('cpOk');
    ...
    api('change-password', 'POST', { current_password: currentPass, new_password: newPass }).then(function(d) {
      if (d.error) { errEl.textContent = d.error; errEl.className = 'auth-error show'; return; }
      okEl.style.display = '';
  ...
  function logout() {
    ...
    if (landingPage) landingPage.className = 'screen active';
  ...
  function enterApp() {
    ...
    var payStatus = urlParams.get('payment');
    var payReqId = urlParams.get('pr');
    if (payStatus === 'success' && payReqId) {
      // Confirm payment on server, then show Payments tab
      api('confirm-payment-request', 'POST', { payment_request_id: payReqId }).then(function(d) {
        if (d.ok) {
    ...
    loadDashboard();
    // Reset guest parking fields for logged-in users
    window._guestParking = false;
    var gi = document.getElementById('pkGuestInfo');
  ...
  function goBack() {
    // If in guest parking mode, go back to landing page
    if (window._guestParking) {
      window._guestParking = false;
      document.getElementById('appShell').style.display = 'none';
      document.getElementById('landingPage').className = 'screen active';
      var nav = document.getElementById('bottomNav');
  ...
  function updateNav(page) {
    var navMap = { dashboard:'dashboard', maintenance:'maintenance', maintenanceNew:'maintenance',
      ...
      packages:'packages', payments:'payments', info:'info', services:'services', serviceList:'services', serviceBook:'services', serviceConfirm:'services' };
  ...
  function loadDashboard() {
    ...
    api('dashboard').then(function(d) {
      ...
      if (d.packages_pending > 0) {
        ...
        setNavBadge('packages', d.packages_pending);
      } else {
        document.getElementById('dashAlert').style.display = 'none';
      }

      // ── Parking (expiring within 3 days) ──
      if (d.parking_active) {
        var endDate = d.parking_active.end_date || '';
        var daysLeft = endDate ? Math.ceil((new Date(endDate) - new Date()) / 86400000) : 999;
        if (daysLeft <= 3 && daysLeft >= 0) {
          setTag('parking', daysLeft === 0 ? 'Expires today!' : 'Expires in ' + daysLeft + 'd', 'danger');
          setNavBadge('parking', '!');
        } else {
          setTag('parking', 'Active', 'success');
        }
      }
      ...
      if (d.maintenance_open > 0) {
        ...
        setNavBadge('maintenance', d.maintenance_open);
      }
      ...
      if (USER.user_type !== 'limited') {
        var pUnit = (USER.unit || USER.apt || '').replace(/\s+/g, ' ').trim();
        var pUserId = USER.user_id || '';
        // Query by unit and/or user_id
        var payFetches = [];
        if (pUnit) payFetches.push(fetch(SB_URL + '/rest/v1/payment_requests?select=id,amount&unit=eq.' + encodeURIComponent(pUnit) + '&status=eq.pending', { headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + SB_KEY } }).then(function(r) { return r.json(); }).catch(function() { return []; }));
        if (pUserId) payFetches.push(fetch(SB_URL + '/rest/v1/payment_requests?select=id,amount&user_id=eq.' + encodeURIComponent(pUserId) + '&status=eq.pending', { headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + SB_KEY } }).then(function(r) { return r.json(); }).catch(function() { return []; }));
        if (payFetches.length > 0) {
          Promise.all(payFetches).then(function(results) {
            var all = []; results.forEach(function(r) { if (Array.isArray(r)) all = all.concat(r); });
            var seen = {}; var rows = all.filter(function(r) { if (seen[r.id]) return false; seen[r.id] = true; return true; });
            if (rows.length > 0) {
              var bal = 0;
              rows.forEach(function(r) { bal += parseFloat(r.amount || 0); });
              setTag('payments', '$' + bal.toFixed(2) + ' due', 'danger');
              setBadge('payments', rows.length);
              setNavBadge('payments', rows.length);
            } else {
              setTag('payments', 'Paid', 'success');
            }
          }).catch(function() {
            if (d.balance && d.balance > 0) {
              setTag('payments', '$' + parseFloat(d.balance).toFixed(2) + ' due', 'danger');
              setNavBadge('payments', '!');
            } else {
              setTag('payments', 'Paid', 'success');
            }
          });
  ...
  function setNavBadge(tab, count) {
    var el = document.getElementById('navBadge_' + tab);
  ...
  function submitMaintenance() {
    ...
    if (USER) {
      body.user_id = USER.user_id || USER.id || null;
  ...
  function bookParking() {
    ...
    if (window._guestParking) {
      var gFirst = document.getElementById('pkGuestFirst').value.trim();
      var gLast = document.getElementById('pkGuestLast').value.trim();
      var gApt = document.getElementById('pkGuestApt').value.trim();
      if (!gFirst || !gLast || !gApt) {
        errEl.textContent = 'First name, last name, and apartment number are required.';
        errEl.className = 'auth-error show';
        ['pkGuestFirst','pkGuestLast','pkGuestApt'].forEach(function(id) {
      ...
      var notifyType = document.getElementById('pkNotifyType').value;
      if (!notifyType) {
        errEl.textContent = 'Please select how to receive your confirmation (email or SMS).';
        errEl.className = 'auth-error show';
        return;
      }
      var notifyValue = '';
      if (notifyType === 'email') {
        notifyValue = document.getElementById('pkNotifyEmail').value.trim();
        if (!notifyValue) {
          errEl.textContent = 'Please enter your email address.';
          errEl.className = 'auth-error show';
          document.getElementById('pkNotifyEmail').style.borderColor = '#e74c3c';
          return;
        }
      } else if (notifyType === 'sms') {
        notifyValue = document.getElementById('pkNotifyPhone').value.trim();
        if (!notifyValue) {
          errEl.textContent = 'Please enter your phone number.';
          errEl.className = 'auth-error show';
          document.getElementById('pkNotifyPhone').style.borderColor = '#e74c3c';
          return;
        }
        var smsConsent = document.getElementById('pkSmsConsent');
        if (smsConsent && !smsConsent.checked) {
          errEl.textContent = 'Please accept the SMS disclaimer to continue.';
          errEl.className = 'auth-error show';
          return;
        }
      }
    ...
    // Add guest info if in guest mode
    if (window._guestParking) {
      _pkPendingBooking.guest_first = document.getElementById('pkGuestFirst').value.trim();
      _pkPendingBooking.guest_last = document.getElementById('pkGuestLast').value.trim();
      _pkPendingBooking.guest_apt = document.getElementById('pkGuestApt').value.trim();
      _pkPendingBooking.guest_name = _pkPendingBooking.guest_first + ' ' + _pkPendingBooking.guest_last;
      _pkPendingBooking.notify_type = document.getElementById('pkNotifyType').value;
      _pkPendingBooking.notify_value = _pkPendingBooking.notify_type === 'email'
        ? document.getElementById('pkNotifyEmail').value.trim()
        : document.getElementById('pkNotifyPhone').value.trim();
      _pkPendingBooking.is_guest = true;
    }
  ...
  function loadPayments() {
    ...
    el.innerHTML = '<div class="info-loading">Loading...</div>';

    // Load payment requests from Supabase first, then build the page
    var unit = (USER.unit || USER.apt || '').replace(/\s+/g, ' ').trim();
    var userId = USER.user_id || '';

    // Query by unit OR user_id, merge results
    var fetchByUnit = unit
      ? fetch(SB_URL + '/rest/v1/payment_requests?unit=eq.' + encodeURIComponent(unit) + '&order=created_at.desc', { headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + SB_KEY } }).then(function(r) { return r.json(); }).catch(function() { return []; })
      : Promise.resolve([]);
    var fetchByUser = userId
      ? fetch(SB_URL + '/rest/v1/payment_requests?user_id=eq.' + encodeURIComponent(userId) + '&order=created_at.desc', { headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + SB_KEY } }).then(function(r) { return r.json(); }).catch(function() { return []; })
      : Promise.resolve([]);

    var fetchPay = Promise.all([fetchByUnit, fetchByUser]).then(function(results) {
      var all = (results[0] || []).concat(results[1] || []);
      // Deduplicate by id
      var seen = {};
      return all.filter(function(r) { if (seen[r.id]) return false; seen[r.id] = true; return true; });
    });

    fetchPay.then(function(rows) {
      if (!Array.isArray(rows)) rows = [];

      // Calculate balance from pending payment requests
      var pending = rows.filter(function(r) { return r.status === 'pending'; });
      var balance = 0;
      pending.forEach(function(r) { balance += parseFloat(r.amount || 0); });

      // Build page HTML
      var html = '';

      // Balance summary — driven by actual Supabase data
      // (rent invoices add to this balance asynchronously via _loadRentInvoices)
      html += '<div class="pay-summary">' +
        '<div class="pay-label">Balance Due</div>' +
        '<div class="pay-amount" id="payBalanceAmount">$' + balance.toFixed(2) + '</div>' +
        '</div>';
      // Cache the request-only balance so rent loader can add to it
      window._paymentRequestBalance = balance;

      if (balance > 0) {
        html += '<div style="text-align:center;padding:8px 0;color:#d32f2f;font-weight:600">⚠️ You have ' + pending.length + ' pending payment' + (pending.length > 1 ? 's' : '') + '</div>';
      } else {
        html += '<div style="text-align:center;padding:8px 0;color:#2e7d32;font-weight:600">✅ No balance due</div>';
      }

      // Payment requests section (cleaning, utilities, ad-hoc charges) — shown first
      html += '<div class="section-label mt-16" style="padding:0">PAYMENT REQUESTS</div><div id="payRequestsSection"></div>';

      // Rent invoices section (client view of monthly rent invoices from PropDesk)
      html += '<div class="section-label mt-16" style="padding:0">RENT</div><div id="rentInvoicesSection"><div class="info-loading">Loading rent invoices…</div></div>';
      ...
      // Render payment request cards
      _renderPayRequests(rows);

      // Load + render rent invoices (monthly invoices from PropDesk)
      _loadRentInvoices();
  ...
  function _loadPaymentRequests() {
    var unit = (USER.unit || USER.apt || '').replace(/\s+/g, ' ').trim();
    if (!unit) { _renderPayRequests([]); return; }
    var url = SB_URL + '/rest/v1/payment_requests?unit=eq.' + encodeURIComponent(unit) + '&order=created_at.desc';
    fetch(url, { headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + SB_KEY } })
      .then(function(r) { return r.json(); })
      .then(function(rows) { _renderPayRequests(Array.isArray(rows) ? rows : []); })
      .catch(function() { _renderPayRequests([]); });
  }

  // ── Rent invoices (client view of PropDesk invoices table) ──
  function _loadRentInvoices() {
    var wrap = document.getElementById('rentInvoicesSection');
    if (!wrap) return;
    var unit = (USER.unit || USER.apt || '').replace(/\s+/g, ' ').trim();
    var property = USER.address || USER.property || '';
    if (!unit) { wrap.innerHTML = '<div style="text-align:center;padding:16px 0;color:var(--text-secondary);font-size:13px">No unit on file — can\'t load rent invoices.</div>'; return; }

    var q = SB_URL + '/rest/v1/invoices?select=id,property,unit,period_month,due_date,status,total,paid,notes'
          + '&unit=eq.' + encodeURIComponent(unit)
          + (property ? '&property=eq.' + encodeURIComponent(property) : '')
          + '&order=period_month.desc';
    fetch(q, { headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + SB_KEY } })
      .then(function(r) { return r.ok ? r.json() : []; })
      .then(function(rows) {
        if (!Array.isArray(rows) || !rows.length) {
          wrap.innerHTML = '<div style="text-align:center;padding:16px 0;color:var(--text-secondary);font-size:13px">No rent invoices yet.</div>';
          return;
        }
        // Fetch payments for these invoices in bulk
        var idList = rows.map(function(i) { return '"' + i.id + '"'; }).join(',');
        return fetch(SB_URL + '/rest/v1/payments?invoice_id=in.(' + idList + ')&select=invoice_id,amount,status,paid_at', {
          headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + SB_KEY }
        }).then(function(r) { return r.ok ? r.json() : []; })
        .then(function(pays) {
          var payByInv = {};
          (pays || []).forEach(function(p) { if (!payByInv[p.invoice_id]) payByInv[p.invoice_id] = 0;
            if ((p.status || 'succeeded') === 'succeeded') payByInv[p.invoice_id] += parseFloat(p.amount||0);
          });
          _renderRentInvoices(rows, payByInv);
        });
      })
      .catch(function(e) {
        console.warn('[rentInvoices] load failed', e);
        wrap.innerHTML = '<div style="text-align:center;padding:16px 0;color:#d32f2f;font-size:13px">Failed to load rent invoices.</div>';
      });
  }

  // Cache of enriched rent rows (for toggle re-renders)
  var _rentCache = { rows: [], groups: null };
  // Section-level expand state
  var _rentShowAllUpcoming = false;
  var _rentShowHistory = false;

  function _isAutopay(inv) {
    return !!(inv && inv.notes && /\[AUTOPAY\]/i.test(String(inv.notes)));
  }

  function _classifyInvoice(inv, payByInv) {
    var today = new Date(); today.setHours(0,0,0,0);
    var paid = payByInv[inv.id] || 0;
    var total = parseFloat(inv.total || 0);
    var remaining = Math.max(0, total - paid);
    var dbStatus = (inv.status || 'open').toLowerCase();
    var due = inv.due_date ? new Date(inv.due_date + 'T12:00:00') : null;
    var status = dbStatus, daysLate = 0;
    ...
    return { inv: inv, paid: paid, total: total, remaining: remaining, status: status, daysLate: daysLate, due: due, autopay: _isAutopay(inv) };
  }

  function _renderRentInvoices(rows, payByInv) {
    var wrap = document.getElementById('rentInvoicesSection');
    if (!wrap) return;

    // Classify + bucket
    var enriched = rows.map(function(inv) { return _classifyInvoice(inv, payByInv); });
    var pastDue  = enriched.filter(function(r) { return r.status === 'late' || r.status === 'partial' || r.status === 'open'; });
    var upcoming = enriched.filter(function(r) { return r.status === 'upcoming'; })
                           .sort(function(a,b){ return (a.due || 0) - (b.due || 0); }); // soonest first
    var history  = enriched.filter(function(r) { return r.status === 'paid' || r.status === 'void'; });

    _rentCache = { enriched: enriched, pastDue: pastDue, upcoming: upcoming, history: history };

    // Balance includes past-due only (not upcoming / paid / void)
    var rentDueTotal = pastDue.reduce(function(s, r) { return s + r.remaining; }, 0);

    var h = '';

    // Past-due section (always shown, all of them)
    if (pastDue.length) {
      pastDue.forEach(function(r) { h += _renderRentCard(r); });
    }

    // Upcoming section: show 1 by default, all if expanded
    if (upcoming.length) {
      var visibleUpcoming = _rentShowAllUpcoming ? upcoming : upcoming.slice(0, 1);
      visibleUpcoming.forEach(function(r) { h += _renderRentCard(r); });
      if (upcoming.length > 1) {
        h += '<button onclick="PL.toggleUpcomingInvoices()" style="width:100%;margin:4px 0 10px;padding:10px;border:1px solid var(--border);background:transparent;border-radius:8px;color:var(--text);font-size:13px;font-weight:600;cursor:pointer">'
           + (_rentShowAllUpcoming ? '▲ Hide upcoming' : '▼ Show all upcoming (' + upcoming.length + ')')
           + '</button>';
      }
    } else if (!pastDue.length) {
      h += '<div style="text-align:center;padding:16px 0;color:var(--text-secondary);font-size:13px">No rent invoices due.</div>';
    }

    // History toggle
    if (history.length) {
      h += '<button onclick="PL.toggleRentHistory()" style="width:100%;margin:4px 0 8px;padding:10px;border:1px solid var(--border);background:transparent;border-radius:8px;color:var(--text);font-size:13px;font-weight:600;cursor:pointer">'
         + (_rentShowHistory ? '▲ Hide payment history' : '▼ Payment history (' + history.length + ')')
         + '</button>';
      if (_rentShowHistory) {
        history.forEach(function(r) { h += _renderRentCard(r); });
      }
    }

    wrap.innerHTML = h;

    // Add rent balance to the top-of-page Balance Due
    var bal = document.getElementById('payBalanceAmount');
    if (bal) {
      var combined = (window._paymentRequestBalance || 0) + rentDueTotal;
      bal.textContent = '$' + combined.toFixed(2);
    }
  }

  function _renderRentCard(r) {
    var inv = r.inv;
    var period = inv.period_month ? new Date(inv.period_month + 'T12:00:00') : null;
    var periodLbl = period ? period.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) : 'Invoice';
    var dueLbl = inv.due_date || '—';
    var status = r.status;
    var color = status === 'late' ? '#d32f2f'
              : status === 'paid' ? '#2e7d32'
              : status === 'partial' ? '#1a56db'
              : status === 'upcoming' ? '#6b7280'
              : status === 'void' ? '#8590a8'
              : '#b86818';
    var badgeText = status === 'late' ? (r.daysLate + ' days late') : (status.charAt(0).toUpperCase() + status.slice(1));
    var autoBadge = r.autopay
      ? '<span style="display:inline-block;margin-left:8px;padding:2px 8px;background:#e8f1ff;color:#1a56db;border-radius:10px;font-size:10px;font-weight:700;letter-spacing:.5px">⚡ AUTO</span>'
      : '';

    var actionBtn;
    if (status === 'paid' || status === 'void') {
      actionBtn = '<div style="font-size:12px;color:' + color + ';font-weight:600;margin-top:6px">' + (status === 'paid' ? '✓ Paid in full' : 'Voided') + '</div>';
    } else if (status === 'upcoming' && r.autopay) {
      actionBtn = '<button onclick="PL.openAutopayInvoice(\'' + esc(inv.id) + '\')" style="width:100%;margin-top:10px;padding:10px;border:1px solid #1a56db;border-radius:8px;background:#fff;color:#1a56db;font-weight:700;font-size:14px;cursor:pointer">⚡ Scheduled · $' + r.remaining.toFixed(2) + '</button>';
    } else if (status === 'upcoming') {
      actionBtn = '<button onclick="PL.openRentInvoice(\'' + esc(inv.id) + '\')" style="width:100%;margin-top:10px;padding:10px;border:none;border-radius:8px;background:' + color + ';color:#fff;font-weight:700;font-size:14px;cursor:pointer">Pay early — $' + r.remaining.toFixed(2) + '</button>';
    } else {
      actionBtn = '<button onclick="PL.openRentInvoice(\'' + esc(inv.id) + '\')" style="width:100%;margin-top:10px;padding:10px;border:none;border-radius:8px;background:' + color + ';color:#fff;font-weight:700;font-size:14px;cursor:pointer">View & Pay $' + r.remaining.toFixed(2) + '</button>';
    }

    return '<div style="background:var(--card-bg);border-radius:12px;padding:14px;margin-bottom:10px;border-left:4px solid ' + color + '">' +
      '<div style="display:flex;justify-content:space-between;align-items:flex-start">' +
        '<div style="flex:1;min-width:0">' +
          '<div style="font-weight:600;font-size:14px">Rent — ' + esc(periodLbl) + autoBadge + '</div>' +
          '<div style="font-size:12px;color:var(--text-secondary);margin-top:2px">Due ' + esc(dueLbl) + '</div>' +
          '<div style="font-size:11px;margin-top:4px;color:' + color + ';font-weight:600">' + esc(badgeText) + '</div>' +
        '</div>' +
        '<div style="text-align:right;flex-shrink:0;margin-left:12px">' +
          '<div style="font-size:18px;font-weight:700;color:' + color + '">$' + r.remaining.toFixed(2) + '</div>' +
          (r.paid > 0 && r.paid < r.total ? '<div style="font-size:11px;color:var(--text-secondary)">of $' + r.total.toFixed(2) + ' · $' + r.paid.toFixed(2) + ' paid</div>' : '') +
        '</div>' +
      '</div>' +
      actionBtn +
    '</div>';
  ...
  function toggleUpcomingInvoices() {
    _rentShowAllUpcoming = !_rentShowAllUpcoming;
    if (_rentCache.enriched) {
      var payByInv = {};
      _rentCache.enriched.forEach(function(r) { payByInv[r.inv.id] = r.paid; });
      _renderRentInvoices(_rentCache.enriched.map(function(r){ return r.inv; }), payByInv);
    }
  }
  function toggleRentHistory() {
    _rentShowHistory = !_rentShowHistory;
    if (_rentCache.enriched) {
      var payByInv = {};
      _rentCache.enriched.forEach(function(r) { payByInv[r.inv.id] = r.paid; });
      _renderRentInvoices(_rentCache.enriched.map(function(r){ return r.inv; }), payByInv);
  ...
  // Click on a scheduled-for-autopay invoice → offer cancel options
  function openAutopayInvoice(invoiceId) {
    var r = (_rentCache.enriched || []).find(function(x){ return x.inv.id === invoiceId; });
    if (!r) { openRentInvoice(invoiceId); return; }
    var period = r.inv.period_month ? new Date(r.inv.period_month + 'T12:00:00') : null;
    var periodLbl = period ? period.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) : 'this invoice';
    var upcomingAuto = (_rentCache.upcoming || []).filter(function(x){ return x.autopay; });

    var msg = '⚡ Scheduled for autopay\n\n'
            + 'Rent — ' + periodLbl + '\n'
            + 'Amount: $' + r.remaining.toFixed(2) + '\n'
            + 'Due: ' + (r.inv.due_date || '—') + '\n\n'
            + 'This invoice will be charged automatically using your saved payment method.\n\n'
            + 'Choose an action:\n'
            + '  OK → Cancel autopay for THIS invoice only\n'
            + '  Cancel → Keep as-is (close)\n\n'
            + 'To cancel ALL ' + upcomingAuto.length + ' upcoming autopay invoices, click OK then confirm the follow-up prompt.';

    if (!confirm(msg)) return;

    // Ask: all or just this?
    var allQ = 'Cancel autopay for ALL ' + upcomingAuto.length + ' upcoming invoice(s)?\n\nOK = cancel ALL upcoming\nCancel = cancel only ' + periodLbl;
    if (confirm(allQ)) {
      _cancelAutopay(upcomingAuto.map(function(x){ return x.inv; }));
    } else {
      _cancelAutopay([r.inv]);
    }
  }

  function _cancelAutopay(invoices) {
    if (!invoices.length) return;
    var pending = invoices.length;
    var errors = 0;
    invoices.forEach(function(inv) {
      var newNotes = (inv.notes || '').replace(/\[AUTOPAY\]\s*/gi, '').trim();
      fetch(SB_URL + '/rest/v1/invoices?id=eq.' + encodeURIComponent(inv.id), {
        method: 'PATCH',
        headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + SB_KEY, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
        body: JSON.stringify({ notes: newNotes })
      }).then(function(r){ if (!r.ok) errors++; })
        .catch(function(){ errors++; })
        .finally(function(){
          pending--;
          if (pending === 0) {
            if (errors) alert('Some autopay cancellations failed (' + errors + '). Please try again.');
            else alert('Autopay canceled for ' + invoices.length + ' invoice(s).');
            _loadRentInvoices();
          }
        });
    });
  ...
  // Open rent invoice in client-mode universal modal
  function openRentInvoice(invoiceId) {
    // Ensure invoice.js has CONFIG set
    if (!window.CONFIG) window.CONFIG = {};
    window.CONFIG.SUPABASE_URL = SB_URL;
    window.CONFIG.SUPABASE_KEY = SB_KEY;
    if (typeof window.WPA_openInvoice === 'function') {
      window.WPA_openInvoice(invoiceId, { mode: 'client' });
    } else {
      alert('Invoice viewer not loaded. Please refresh.');
    }
  }

  function _renderPayRequests(rows) {
    var wrap = document.getElementById('payRequestsSection');
    if (!wrap) return;
    ...
    if (rows.length === 0) {
      wrap.innerHTML = '<div style="text-align:center;padding:20px 0;color:var(--text-secondary);font-size:13px">No payment requests</div>';
      return;
    }
    ...
    if (pending.length) {
      h += '<div class="section-label" style="padding:0;color:#d32f2f;font-weight:700">Action Required</div>';
      pending.forEach(function(r) {
        var amt = parseFloat(r.amount || 0).toFixed(2);
        var rawDesc = r.description || 'Service';
        var descLines = rawDesc.split('\n');
        var title = esc(descLines[0]);
        var wo = r.wo_number ? '<span style="font-size:11px;color:var(--text-secondary);margin-left:6px">WO #' + esc(r.wo_number) + '</span>' : (r.work_order_id ? '<span style="font-size:11px;color:var(--text-secondary);margin-left:6px">WO #' + esc(r.work_order_id) + '</span>' : '');
        var dueStr = r.due_date ? '<div style="font-size:11px;color:#d32f2f;margin-top:2px">Due: ' + r.due_date + '</div>' : '';
        var dateStr = r.created_at ? fmtTime(r.created_at) : '';
        // Build charge breakdown from description lines
        var breakdownHtml = '';
        if (descLines.length > 1) {
          breakdownHtml = '<div style="margin-top:8px;padding:8px 10px;background:rgba(0,0,0,.03);border-radius:6px;font-size:12px">';
          for (var _di = 1; _di < descLines.length; _di++) {
            var ln = descLines[_di].trim();
            if (!ln) continue;
            var parts = ln.split(':');
            if (parts.length >= 2) {
              breakdownHtml += '<div style="display:flex;justify-content:space-between;padding:2px 0"><span style="color:var(--text-secondary)">' + esc(parts[0].trim()) + '</span><strong>' + esc(parts.slice(1).join(':').trim()) + '</strong></div>';
            } else {
              breakdownHtml += '<div style="padding:2px 0;color:var(--text-secondary)">' + esc(ln) + '</div>';
            }
          }
          breakdownHtml += '</div>';
        }
        h += '<div style="background:var(--card-bg);border-radius:12px;padding:14px;margin-bottom:10px;border-left:4px solid #d32f2f">' +
          '<div style="display:flex;justify-content:space-between;align-items:flex-start">' +
            '<div style="flex:1;min-width:0">' +
              '<div style="font-weight:600;font-size:14px">' + title + wo + '</div>' +
              '<div style="font-size:12px;color:var(--text-secondary);margin-top:2px">' + dateStr + '</div>' +
              dueStr +
            '</div>' +
            '<div style="text-align:right;flex-shrink:0;margin-left:12px">' +
              '<div style="font-size:18px;font-weight:700;color:#d32f2f">$' + amt + '</div>' +
            '</div>' +
          '</div>' +
          breakdownHtml +
          '<button onclick="PL.viewPayRequest(\'' + esc(r.id) + '\')" style="width:100%;margin-top:10px;padding:10px;border:none;border-radius:8px;background:#d32f2f;color:#fff;font-weight:700;font-size:14px;cursor:pointer">View & Pay $' + amt + '</button>' +
        '</div>';
      });
    }
    ...
    // Paid — completed
    if (paid.length) {
      h += '<div class="section-label" style="padding:0;margin-top:' + (pending.length ? '16px' : '0') + '">Paid</div>';
      paid.forEach(function(r) {
        ...
        var title = esc(descLines[0]);
        var wo = r.wo_number ? '<span style="font-size:11px;color:var(--text-secondary);margin-left:6px">WO #' + esc(r.wo_number) + '</span>' : (r.work_order_id ? '<span style="font-size:11px;color:var(--text-secondary);margin-left:6px">WO #' + esc(r.work_order_id) + '</span>' : '');
        var paidDate = r.paid_at ? fmtTime(r.paid_at) : fmtTime(r.updated_at || r.created_at);
        var breakdownHtml = '';
        if (descLines.length > 1) {
          breakdownHtml = '<div style="margin-top:8px;padding:8px 10px;background:rgba(0,0,0,.03);border-radius:6px;font-size:12px">';
          for (var _pi = 1; _pi < descLines.length; _pi++) {
            var ln = descLines[_pi].trim();
            if (!ln) continue;
            var parts = ln.split(':');
            if (parts.length >= 2) {
              breakdownHtml += '<div style="display:flex;justify-content:space-between;padding:2px 0"><span style="color:var(--text-secondary)">' + esc(parts[0].trim()) + '</span><strong>' + esc(parts.slice(1).join(':').trim()) + '</strong></div>';
        ...
        h += '<div style="background:var(--card-bg);border-radius:12px;padding:14px;margin-bottom:10px;border-left:4px solid #2e7d32">' +
          '<div style="display:flex;justify-content:space-between;align-items:center">' +
            '<div style="flex:1;min-width:0">' +
              '<div style="font-weight:600;font-size:14px">' + title + wo + '</div>' +
              '<div style="font-size:12px;color:var(--text-secondary);margin-top:2px">Paid ' + paidDate + '</div>' +
            '</div>' +
            '<div style="flex-shrink:0;margin-left:12px">' +
              '<div style="display:inline-flex;align-items:center;gap:4px;background:#e8f5e9;color:#2e7d32;padding:4px 10px;border-radius:20px;font-weight:700;font-size:13px">' +
                '<span style="font-size:14px">&#10003;</span> $' + amt +
              '</div>' +
            '</div>' +
          '</div>' +
          breakdownHtml +
    ...
    // Cancelled
    if (cancelled.length) {
      h += '<div class="section-label" style="padding:0;margin-top:16px;color:var(--text-secondary)">Cancelled</div>';
      cancelled.forEach(function(r) {
        var amt = parseFloat(r.amount || 0).toFixed(2);
        var desc = esc(r.description || 'Service');
        h += '<div style="background:var(--card-bg);border-radius:12px;padding:14px;margin-bottom:10px;opacity:0.6">' +
          '<div style="display:flex;justify-content:space-between;align-items:center">' +
            '<div><div style="font-weight:600;font-size:14px;text-decoration:line-through">' + desc + '</div></div>' +
            '<div style="font-size:13px;color:var(--text-secondary)">$' + amt + '</div>' +
          '</div>' +
          (r.cancel_reason ? '<div style="font-size:11px;color:var(--text-secondary);margin-top:4px">Reason: ' + esc(r.cancel_reason) + '</div>' : '') +
        '</div>';
      });
    }
  ...
  // Open a payment_request in the unified invoice modal (same design as rent).
  // Modal's Pay button routes to pay.php for the tip screen + Stripe.
  function viewPayRequest(payId) {
    if (!window.CONFIG) window.CONFIG = {};
    window.CONFIG.SUPABASE_URL = SB_URL;
    window.CONFIG.SUPABASE_KEY = SB_KEY;
    window.WPA_PORTAL_URL = (location.pathname.indexOf('/portal') >= 0)
      ? location.origin + location.pathname.replace(/\/portal.*/, '/portal')
      : location.origin + '/portal';
    if (typeof window.WPA_openPaymentRequest === 'function') {
      window.WPA_openPaymentRequest(payId);
    } else {
      // Fallback to the legacy direct-to-pay.php flow
      payRequest(payId);
    }
  }

  function payRequest(payId) {
    // Fetch payment request details and route through tip-enabled pay.php
    var url = SB_URL + '/rest/v1/payment_requests?id=eq.' + encodeURIComponent(payId) + '&select=id,stripe_invoice_url,amount,description,work_order_id';
    fetch(url, { headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + SB_KEY } })
      .then(function(r) { return r.json(); })
      .then(function(rows) {
        if (!rows || !rows.length) { toast('Payment request not found'); return; }
        var req = rows[0];
        var amtCents = Math.round((parseFloat(req.amount) || 0) * 100);
        var desc = (req.description || 'Service').split('\n')[0]; // first line only
        var wo = req.work_order_id || '';
        // Route through tip-enabled payment page
        var payUrl = '/pay.php'
          + '?amount=' + amtCents
      ...
      .catch(function() { toast('Error loading payment details.'); });
  }
  ...
  function openServiceBook(serviceId) {
    ...
    if (svc.pricing_type === 'configurable' && svc.pricing_config) {
      ...
      if (!selectors.length && cfg.base_bedrooms !== undefined) {
        ...
        cfg.selectors = selectors; // persist so price calc + submission can find them
        if (!cfg.base_price && cfg.base_price !== 0) cfg.base_price = svc.base_price || 0;
      }
  ...
  function submitServiceBooking() {
    ...
    // Add user info — pre-assign property/unit from logged-in client
    if (USER) {
      payload.name = USER.name || '';
      payload.phone = USER.phone || '';
      payload.email = USER.email || '';
      payload.unit = USER.unit || USER.apt || '';
      payload.user_id = USER.user_id || USER.id || null;
      payload.submitted_by = USER.name || '';
      if (USER.address) { payload.address = USER.address; payload.property = USER.address; }
    ...
    api('maintenance-submit-hs', 'POST', payload).then(function(d) {
  ...
  return {
    ...
    pkToggleNotifyInput: pkToggleNotifyInput,
    ...
    openRentInvoice: openRentInvoice,
    openAutopayInvoice: openAutopayInvoice,
    toggleUpcomingInvoices: toggleUpcomingInvoices,
    toggleRentHistory: toggleRentHistory,
