...
(function() {
  ...
  // -- Normalize a resident name for matching --
  function normName(n) {
    return (n || '').toLowerCase().replace(/[^a-z]/g, '');
  }

  // -- Find the best grouping key for a message --
  // Uses email when available; falls back to name-based matching against
  // residents we already know, so messages without email still merge.
  function residentKey(m, resMap) {
    if (m.resident_email) return m.resident_email.toLowerCase().trim();
    // No email -- try to match by normalized name to an existing keyed resident
    var mName = normName(m.resident_name);
    if (resMap && mName) {
      var keys = Object.keys(resMap);
      for (var i = 0; i < keys.length; i++) {
        if (normName(resMap[keys[i]].from) === mName) return keys[i];
      }
    }
    return 'name:' + mName + '|' + (m.resident_unit || '').trim();
  }
  ...
  window._refreshClientMsgs = async function() {
    try {
      ...
      // Process messages with email first so name-only messages can merge in
      data.sort(function(a, b) {
        var ae = a.resident_email ? 0 : 1;
        var be = b.resident_email ? 0 : 1;
        return ae - be;
      });
      var resMap = {};
      data.forEach(function(m) {
        var key = residentKey(m, resMap);
        if (!resMap[key]) {
          resMap[key] = {
        ...
        // Fill in missing contact info from any message that has it
        if (!entry._email && m.resident_email) entry._email = m.resident_email;
        if (!entry.unit && m.resident_unit) entry.unit = m.resident_unit;
        if (!entry._phone && m.resident_phone) entry._phone = m.resident_phone;
      ...
      // Re-sort messages within each group by created_at ascending
      Object.values(resMap).forEach(function(entry) {
        entry._messages.sort(function(a, b) {
  ...
  // Also deduplicates across sources so same resident = ONE entry
  ...
  window._getAllCenterMessages = function() {
    ...
    // Build combined list starting with live Supabase client messages (priority)
    var combined = [];
    if (window._liveClientMsgs) {
      window._liveClientMsgs.forEach(function(m) {
        combined.push(Object.assign({}, m));
      });
    }
    // Merge non-client entries: if a resident already has a client entry (by name or email), absorb into it
    nonClient.forEach(function(nc) {
      var ncName = normName(nc.from);
      var ncEmail = nc._email || nc.email || '';
      var match = null;
      for (var i = 0; i < combined.length; i++) {
        var c = combined[i];
        // Match by email
        if (ncEmail && c._email && ncEmail.toLowerCase().trim() === c._email.toLowerCase().trim()) {
          match = c; break;
        }
        // Match by normalized name
        if (ncName && normName(c.from) === ncName) {
          match = c; break;
        }
      }
      if (match) {
        // Absorb: add the non-client message info into the existing entry
        if (!match._mergedSources) match._mergedSources = [match.source];
        match._mergedSources.push(nc.source);
        // If the non-client entry has a more recent date, update preview
        if (nc.date && new Date(nc.date) > new Date(match.date)) {
          match.date = nc.date;
          match.body = nc.body;
          match.subject = nc.subject || match.subject;
        }
        // Fill in missing contact info
        if (!match._email && ncEmail) match._email = ncEmail;
        if (!match.unit && nc.unit) match.unit = nc.unit;
        if (!match._phone && nc._phone) match._phone = nc._phone;
      } else {
        // No matching client entry â keep as separate entry
        combined.push(nc);
      }
    });
  ...
  window.openComposeMessage = function() {
    ...
    document.getElementById('compose-send-btn').addEventListener('click', async function() {
      ...
      // Auto-fill missing fields from existing resident data
      if (!unit || !email) {
        var nameLower = name.toLowerCase().trim();
        var match = (window._liveClientMsgs || []).find(function(m) {
          if (email && m._email && m._email.toLowerCase().trim() === email.toLowerCase().trim()) return true;
          return m.from && m.from.toLowerCase().trim() === nameLower;
        });
        if (match) {
          if (!unit && match.unit) unit = match.unit;
          if (!email && match._email) email = match._email;
          if (!phone && match._phone) phone = match._phone;
          if (!property && match.property) property = match.property;
        }
      }
  ...
  // If an existing chat with that resident exists, open it; otherwise compose new.
  ...
  window.openMsgModal = function(name, email, phone, bookingId, type) {
    ...
    setTimeout(function() {
      // Try to find existing chat by email first, then by name
      var existing = null;
      var msgs = window._liveClientMsgs || [];
      if (email) {
        var emailLower = email.toLowerCase().trim();
        existing = msgs.find(function(m) {
          return m._email && m._email.toLowerCase().trim() === emailLower;
        });
      }
      if (!existing && name) {
        var nameLower = name.toLowerCase().trim();
      ...
      if (existing) {
        // Open existing chat thread
        if (typeof openMsgCenterDetail === 'function') openMsgCenterDetail(existing.id);
      } else {
        // No existing chat -- open compose pre-filled
