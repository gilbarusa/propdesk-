    ...
    ">
      ...
      ">
        ...
        <div style="font-size:12px; color:var(--text2); line-height:1.6; margin-bottom:10px; display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
          ...
          <button onclick="openInboxThread('${_escHtml(booking.guest_name).replace(/'/g,"\\\\'")}');" style="padding:4px 12px;border-radius:6px;border:1px solid var(--accent);background:var(--accent-bg);color:var(--accent2);font-size:11px;font-weight:600;cursor:pointer;font-family:inherit;">💬 Message</button>
        </div>
...
function renderGuestFormDetail(f) {
  ...
  if (f.id_file_path) {
    // Serve ID images through standalone PHP file (GoDaddy WAF blocks API query strings)
    const idFilename = f.id_file_path.split('/').pop();
    const idUrl = 'https://app.willowpa.com/view-id.php?f=' + encodeURIComponent(idFilename);
    html += `<div style="font-size:12px; color:var(--text2); line-height:2;">
