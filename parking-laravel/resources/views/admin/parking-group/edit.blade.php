...
@section('content')
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0 font-size-18">Edit Parking Group: {{ $group->name }}</h4>
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('parking-group.update', $group->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                    ...
                                    <div class="col-lg-4 mb-3">
                                        ...
                                        <select class="form-control select2" name="building_id" id="building_id" required>
                                            @foreach ($buildings as $building)
                                                <option value="{{ $building->id }}" @selected($building->id == $group->building_id)>
                                                    {{ $building->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div id="periods">
                                    @foreach ($group->plans as $index => $plan)
                                        <div class="row mb-3 period-section">
                                            <div class="col-md-2">
                                                <label>Period {{ $index + 1 }}</label>
                                                <input type="number" name="periods[{{ $index }}][days]"
                                                       value="{{ $plan->days }}" class="form-control">
                                            </div>
                                            <div class="col-md-2">
                                                <label>Price ($)</label>
                                                <input type="number" name="periods[{{ $index }}][price]"
                                                       value="{{ $plan->price }}" class="form-control" step="0.01">
                                            </div>
                                            <div class="col-md-2">
                                ...
                                <div id="units-container" class="row mb-4">
                                    @php
                                        $assignedIds = $group->units->pluck('id')->toArray();
                                    @endphp
                                        ...
                                        <div class="col-md-3 mb-2">
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="unit_ids[]"
                                                       ...
                                                       @checked(in_array($unit->id, $assignedIds))>
                                                <label class="form-check-label" for="unit-{{ $unit->id }}">
                                                    {{ $unit->unit_number }}
                                                    @if ($unit->parking_group_id && $unit->parking_group_id != $group->id)
                                                        <span class="text-warning">(in another group)</span>
                                                    @endif
                                                </label>
                                            </div>
                                        </div>
                                    ...
                                    @if ($units->isEmpty())
                                        <div class="col-12">
                                            <p class="text-muted">No units found for this building.</p>
                                        </div>
                                    @endif
                                </div>

                                <div class="mb-3">
                                    <button type="submit" class="btn btn-primary w-md">Update Group</button>
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Reload units when building changes
            $('#building_id').change(function() {
                ...
                $.get('/parking-group/units-by-building/' + buildingId, function(units) {
                    let html = '';
                    if (units.length === 0) {
                        html = '<div class="col-12"><p class="text-muted">No units found.</p></div>';
                    } else {
                        units.forEach(function(unit) {
                            let inOther = (unit.parking_group_id && unit.parking_group_id != {{ $group->id }})
                                ? ' <span class="text-warning">(in another group)</span>' : '';
                            html += `
                                <div class="col-md-3 mb-2">
                                    <div class="form-check">
